...
function Paragraph({ image, index, offset, factor, header, aspect, text }) {
  ...
  const left = !(index % 2)
  const color = index % 2 ? "#FFA07A" : "#E9967A"
  return (
  ...
  return (
    <>
      <Block factor={1} offset={0}>
        <Block factor={1.2}>
          <Text left size={w * 0.05} position={[-w / 3.2, 2, -1]} color="#fff">
            Hi There,
          </Text>
          <Text left size={w * 0.03} position={[-w / 3.2, 0.5, -1]} color="#fff">
            I'm Bahareh!
          </Text>
        </Block>
        <Block factor={1.0}>
          <Dom position={[-w / 3.2, -w * 0.08 + 0.25, -1]}>Web Developer and designer based in Vancouver!<br /> Scroll down to see my works!</Dom>
        </Block>
      </Block>
      <Block factor={1.2} offset={5.7}>
        <MultilineText top left size={w * 0.10} lineHeight={w / 4} position={[-w / 3.5, 0, -1]} color="#2fe8c3" text={"more\nprojects\nto come"} />
      </Block>
